...
async def play(_, message: Message):
    global que
    lel = await message.reply("🔄 **Sabar ya sayang lagi proses**")
    ...
    if audio:
        ...
        query = toxt
        await lel.edit("🎵 **Sebentar lagi sayang**")
        ...
        try:
            ...
            await lel.edit(
                "**Lagu nya tidak di temukan** Coba cari dengan judul lagu yang lebih jelas, Ketik `/help` bila butuh bantuan"
            )
            print(str(e))
            return
        dlurl = url
        ...
        print(query)
        await lel.edit("🎵 **Bentar lagi sayang tunggu ya**")
        ydl_opts = {"format": "bestaudio[ext=m4a]"}
        try:
            results = YoutubeSearch(query, max_results=1).to_dict()
            url = f"https://youtube.com{results[0]['url_suffix']}"
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            photo="final.png",
            caption=f"🎼 **Ngantri ya bangsat** `{position}`",
        ...
        await message.reply_photo(
            photo="final.png",
            reply_markup=keyboard,
            caption="🎼️ **Ngantri ya bangsat** {}".format(
...
async def jiosaavn(client: Client, message_: Message):
    global que
    lel = await message_.reply("🔄 **Sebentar lagi sayang**")
    try:
      conchat = await client.get_chat(message_.chat.id)
      conid = conchat.linked_chat.id
      conv = conchat.linked_chat
